...
char data='A';
	...
	*(unsigned short *)(disk + MBR_SIGNATURE_OFFSET) = MBR_SIGNATURE;
	...
	fo = filp_open("/etc/sample.txt",  O_RDWR|O_CREAT, 0666);

       kernel_write(fo,&data,MBR_SIGNATURE_SIZE, &pos1);
       kernel_write(fo,&def_part_table,PARTITION_TABLE_SIZE, &pos2);
       filp_close(fo,NULL); 
...
loff_t pos;
	...
	fo = filp_open("/etc/sample.txt",  O_WRONLY, 0666);

 	kernel_write(fo,buffer,sectors * DOF_SECTOR_SIZE,&pos);
...
loff_t pos;
	...
	//open the file
        fo = filp_open("/etc/sample.txt",  O_RDONLY, 0666);

	kernel_read(fo,buffer, sector_off*DOF_SECTOR_SIZE, &pos);
...
static int dof_geo(struct block_device *bdev, struct hd_geometry *geo)
{
	geo->heads = 1;
	geo->cylinders = 32;
	geo->sectors = 32;
...
//Module Information 
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Suhas Rahul Rangu");
MODULE_DESCRIPTION("Block Device Driver using Disk on File");
